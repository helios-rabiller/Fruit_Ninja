...
class HalfFruit:
    def __init__(self, x, y, image, velocity):
        ...
        self.shape.collision_type = 2  # Unique collision type for HalfFruit
        self.image = image
        SPACE.add(self.body, self.shape)
    ...
    def draw(self, window):
        pos = int(self.body.position.x), int(self.body.position.y)
        window.blit(self.image, self.image.get_rect(center=pos))
# Define a collision handler to ignore collisions with HalfFruit
def no_collision(arbiter, space, data):
    return False  # Ignore the collision
# Add the collision handler to the space
handler = SPACE.add_collision_handler(2, 1)  # HalfFruit (2) vs Fruit (1)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONUP:
                ...
                if restart_button.collidepoint(mouse_pos):
                    input_name()  # Demander le nom du joueur
                    game_over = True
                    waiting = False

                # if menu
                elif menu_button.collidepoint(mouse_pos):
                    input_name()  # ask player name
                    show_menu_screen()
                    waiting = False
...
while game_running:
    ...
    if time_paused == False:
        SPACE.step(1 / 70)  # smooth physics
